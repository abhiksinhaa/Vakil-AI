...
export default function FeedbackPopup({
  ...
  return (
    <div
      className={`fixed inset-x-0 bottom-0 z-[140] p-4 transition-all duration-300 ease-out ${
        ...
        <div className="space-y-4">
            ...
            {stars.map((value) => (
              <button
                ...
                className={`rounded-full p-3 transition ${
                  rating >= value ? 'bg-gold text-navy' : 'bg-white/5 text-cream/70 hover:bg-white/10'
                }`}
          ...
          <div className="flex flex-col gap-3">
            <button
              ...
              onClick={onSubmit}
              disabled={loading}
              className="inline-flex items-center justify-center rounded-3xl bg-gold px-5 py-3 text-sm font-semibold text-navy transition hover:bg-[#ffd966] disabled:cursor-not-allowed disabled:opacity-60"
            >
